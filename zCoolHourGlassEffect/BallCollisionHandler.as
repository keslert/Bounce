package  {
	...
	public class BallCollisionHandler {
		...
		private var g_v3:Vector2;
		private var g_vn:Vector2;
		private var g_v4:Vector2;
		...
		//main function
		public function runCheck(balls:Array){
			var l = balls.length;
			var ob:Ball;
			var ob2:Ball;
			for(var i:int=0; i<l; i++){
				...
				for(var j:int=0; j<l;j++){
					...
					if(pen>=0){
						...
						g_v3.p0=ob.p0;
						g_v3.vx=ob.vx-ob2.vx;
						g_v3.vy=ob.vy-ob2.vy;
						updateVector2(g_v3,false);
						//use v3 as new movement vector for collision calculation
						//projection of vc on v3
						var vp:Projection=projectVector2(g_vc, g_v3.dx, g_v3.dy);
						...
						if(diff>0){
							...
							p3.x = g_vn.p0.x-moveBack*g_v3.dx;
							p3.y = g_vn.p0.y-moveBack*g_v3.dy;
							//vector from ball1 starting point to its coordinates when collision happens
							
							g_v4.p0 = ob.p0;
							g_v4.p1 = p3;
							updateVector2(g_v4, true);
							//check if p3 is on the movement vector
							if(g_v4.len<=g_v3.len && dotP(g_v4, ob)>0){
								//collision
								var t=g_v4.len/g_v3.len;
								collision=true;
